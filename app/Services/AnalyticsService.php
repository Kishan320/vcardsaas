...
class AnalyticsService
{
    ...
    {
        $rows = DB::select("
            SELECT device_label, SUM(cnt) as count
            FROM (
                SELECT
                    CASE
                        WHEN NULLIF(`device`, '') IS NOT NULL THEN `device`
                        WHEN `platform` LIKE '%Android%' OR `platform` LIKE '%iOS%'
                             OR `platform` LIKE '%iPhone%' OR `platform` LIKE '%iPad%' THEN 'Mobile'
                        WHEN `platform` LIKE '%Windows%' OR `platform` LIKE '%Mac%'
                             OR `platform` LIKE '%Linux%' THEN 'Desktop'
                        ELSE 'Unknown'
                    END AS device_label,
                    1 AS cnt
        ...
        return collect($rows)->map(fn($r) => (object)['device_type' => $r->device_label, 'count' => $r->count]);
    }
    
    private function getPlatforms($businessId, $startDate, $endDate)
    {
        $rows = DB::select("
            SELECT platform_label, SUM(cnt) as count
            FROM (
                SELECT COALESCE(NULLIF(`platform`, ''), 'Unknown') AS platform_label, 1 AS cnt
                FROM `shetabit_visits`
                WHERE `business_id` = ?
                  AND `created_at` BETWEEN ? AND ?
            ) AS sub
        ...
        return collect($rows)->map(fn($r) => (object)['platform_name' => $r->platform_label, 'count' => $r->count]);
    }
    
    private function getBrowsers($businessId, $startDate, $endDate)
    {
        $rows = DB::select("
            SELECT browser_label, SUM(cnt) as count
            FROM (
                SELECT COALESCE(NULLIF(`browser`, ''), 'Unknown') AS browser_label, 1 AS cnt
                FROM `shetabit_visits`
                WHERE `business_id` = ?
                  AND `created_at` BETWEEN ? AND ?
            ) AS sub
            GROUP BY browser_label
            ORDER BY count DESC
        ", [$businessId, $startDate, $endDate]);

        return collect($rows)->map(fn($r) => (object)['browser_name' => $r->browser_label, 'count' => $r->count]);
    }
    ...
    public function getDeviceAnalytics($businessIds, $dateRange)
    {
        ...
        $businessIdList = $businessIds->implode(',') ?: '0';
        $start = $dateRange[0];
        $end   = $dateRange[1];

        $rows = DB::select("
            SELECT device_label, SUM(cnt) as visitors
            FROM (
                SELECT
                    CASE
                        WHEN NULLIF(`device`, '') IS NOT NULL THEN `device`
                    ...
                    1 AS cnt
                FROM `shetabit_visits`
                WHERE `business_id` IN ({$businessIdList})
                  AND `created_at` BETWEEN ? AND ?
            ) AS sub
            GROUP BY device_label
        ...
        return collect($rows)->map(function ($item) use ($total) {
            return [
    ...
    public function getBrowserAnalytics($businessIds, $dateRange)
    {
        ...
        $rows = DB::select("
            SELECT browser_label, SUM(cnt) as visitors
            FROM (
                SELECT COALESCE(NULLIF(`browser`, ''), 'Unknown') AS browser_label, 1 AS cnt
                FROM `shetabit_visits`
                WHERE `business_id` IN ({$businessIdList})
    ...
    public function getReferrerAnalytics($businessIds, $dateRange)
    {
        ...
        $rows = DB::select("
            SELECT source_label, SUM(cnt) as visitors
            FROM (
                SELECT COALESCE(`referer`, 'Direct') AS source_label, 1 AS cnt
                FROM `shetabit_visits`
                WHERE `business_id` IN ({$businessIdList})
                  AND `created_at` BETWEEN ? AND ?
            ) AS sub
        ...
        return collect($rows)->map(function ($item) use ($total) {
            ...
            return [
                'source'     => $src === 'Direct' ? 'Direct' : (parse_url($src, PHP_URL_HOST) ?? $src),
                'visitors'   => $item->visitors,
                'percentage' => $total > 0 ? round(($item->visitors / $total) * 100, 1) : 0,
            ];
        });
    ...
    {
        return DB::table('shetabit_visits')
            ->whereIn('business_id', $businessIds)
            ->whereBetween('created_at', $dateRange)
