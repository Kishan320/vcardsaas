    ...
    <AppLayout>
        ...
        <div class="space-y-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Bio Link</h1>
                    <p class="text-sm text-gray-500 mt-0.5">{{ bioLink?.title }}</p>
                </div>
            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <Field name="title" :rules="titleSchema" v-model="form.title">
                                        <TextInput v-model="form.title" type="text" :has-error="!!(errors.title || form.errors.title)" class="mt-1.5" />
                                    </Field>
                                ...
                                <div>
                                    ...
                                    <Field name="username" :rules="usernameSchema" v-model="form.username">
                                        <TextInput v-model="form.username" type="text" :has-error="!!(errors.username || form.errors.username)" class="mt-1.5" />
                                    </Field>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-6 bg-white border border-gray-100 rounded-2xl shadow-card p-4">
                        ...
                        <div class="rounded-xl overflow-hidden border border-gray-100 bg-gray-50 p-4 text-center">
                            ...
                            <h4 class="font-bold text-gray-900">{{ form.title }}</h4>
                            <p class="text-xs text-gray-500 mt-1">@{{ form.username }}</p>
...
const socialData = props.bioLink?.social_links ?? {};

const form = useForm({
    title: props.bioLink?.title ?? '',
    username: props.bioLink?.username ?? '',
    bio: props.bioLink?.bio ?? '',
    theme: props.bioLink?.theme ?? 'minimal',
    social_links: {
...
const submit = () => form.put(route('link-bio-builder.update', props.bioLink.id));
</script>
